...
class KMeansApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('K-Means Clustering App')

        # Layouts
        main_layout = QVBoxLayout()
        input_layout = QHBoxLayout()

        # Поля ввода кол-ва точек и кол-ва центроидов
        self.n_samples_input = QLineEdit(self)
        self.n_samples_input.setPlaceholderText("Количество точек (n_samples)")
        self.centers_input = QLineEdit(self)
        self.centers_input.setPlaceholderText("Количество центров (centers)")
        ...
        # Виджет для отрисовки пачки рисунков
        self.stacked_widget = QStackedWidget()

        input_layout.addWidget(self.n_samples_input)
        input_layout.addWidget(self.centers_input)
        ...
        self.num_plots = 3  # 3 Стадии кластеризации

    def run_kmeans(self):
        # Получение параметров из ввода пользователя
        try:
            n_samples = int(self.n_samples_input.text())
            centers = int(self.centers_input.text())
        except ValueError:
            print("Введите корректные целые числа")
            return

        # Генерация данных
        X, _ = make_blobs(n_samples=n_samples, centers=centers, cluster_std=0.60, random_state=0)
        ...
        # Шаг 3: Кластеры и центры
        fig3 = plt.figure()
        ax3 = fig3.add_subplot(111)
        ax3.scatter(X[:, 0], X[:, 1], c=y_kmeans, s=50, cmap='viridis')
        centers = kmeans.cluster_centers_
        ax3.scatter(centers[:, 0], centers[:, 1], c='red', s=200, alpha=0.75, marker='X', label='Centroids')
        ax3.set_title("Шаг 3: Кластеры и центры")
